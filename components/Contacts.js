...
function Contacts() {
  ...
  return (
    ...
    <h3 className={`absolute top-20 font-bold ${styles.headertext} uppercase tracking-[10px] es:tracking-[15px] sm:tracking-[20px] text-white text-4xl`}>Contact me</h3>
    <div className='flex flex-col space-y-10 mt-52'>
      <h4 className='text-4xl font-semibold text-center text-white p-5 md:p-0'>I have what you are looking for{', '}
        <span className='italic text-blue-400'>drop your message  down</span>
      </h4>
      <form ref={form} onSubmit={sendEmail} className='w-fit flex flex-col mx-auto'>
        ...
        <button type='submit' className={`text-xl sm:text-2xl rounded-xl ${styles.submitbtn}  text-white`} >Submit</button>
      </form>
    </div>
    </div>
  )
